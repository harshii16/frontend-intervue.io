...
const TeacherLandingPage = () => {
  ...
  useEffect(() => {
    ...
    console.log("Teacher connecting to socket server at:", apiUrl);
    ...
    socketRef.current.on("connect", () => {
      console.log("Teacher socket connected successfully with ID:", socketRef.current.id);
  ...
  const askQuestion = () => {
    if (validateForm() && socketRef.current && socketConnected) {
      ...
      console.log("Teacher creating poll:", JSON.stringify(pollData, null, 2));
      
      try {
        socketRef.current.emit("createPoll", pollData);
        console.log("Poll creation event emitted");
        
        // Listen for the pollCreated event
        socketRef.current.once("pollCreated", (response) => {
          console.log("Received pollCreated confirmation:", response);
          navigate("/teacher-poll");
        });
      } catch (err) {
        console.error("Error emitting createPoll:", err);
